/*
    ...
    Implementação didática do Pipeline de Alvy Ray Smith
*/
...
// [PIPELINE] Definição Geométrica (Espaço do Objeto)
Vec4 verts_cubo[8] = {
...
int main(int argc, char* argv[]) {
    ...
    SDL_Window* win = SDL_CreateWindow("CG Pipeline - UNIOESTE", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, LARGURA, ALTURA, 0);
    ...
    // Cria um cubo inicial na cena
    Cubo c1; c1.posicao=Vec4(-1.2,0,-5); c1.rotacao=Vec4(0.5,0.6,0); c1.escala=Vec4(1,1,1); c1.cor_base=0xFFFF0000;
    c1.ka=0.2; c1.kd=0.7; c1.ks=0.8; c1.shininess=50; cena.push_back(c1);

    Cubo c2; c2.posicao=Vec4(1.2,0,-5); c2.rotacao=Vec4(0,-0.3,0); c2.escala=Vec4(1,1,1); c2.cor_base=0xFF00FF00;
    c2.ka=0.2; c2.kd=0.7; c2.ks=0.8; c2.shininess=100; cena.push_back(c2);
    
    int sel_idx = 0;
    bool running = true;
    
    std::cout << "--- INICIANDO RENDERIZADOR ---" << std::endl;
    std::cout << "[TAB] Troca Modo | [SPACE] Troca Cubo | [M] Render | [C] Mudar Cor" << std::endl;
        ...
        while(SDL_PollEvent(&e)) {
            ...
            if(e.type==SDL_KEYDOWN) {
                ...
                if(e.key.keysym.sym == SDLK_m) { 
                    ...
                    std::cout << "\n[RENDER] " << (g_use_phong?"PHONG":"FLAT") << std::endl; 
                }
                ...
                // Trocar Cubo Selecionado
                if(e.key.keysym.sym == SDLK_SPACE && !cena.empty()) {
                    sel_idx = (sel_idx+1)%cena.size();
                    std::cout << "\n[SELECAO] Cubo " << sel_idx << std::endl;
                }

                // [RESTORED] Mudar Cor do Cubo (Tecla C)
                if(e.key.keysym.sym == SDLK_c && !cena.empty()) {
                    // Gera cor aleatória: Alpha=255, R, G, B aleatórios
                    cena[sel_idx].cor_base = (255<<24) | (rand()%255<<16) | (rand()%255<<8) | rand()%255;
                    std::cout << "\n[COR] Nova cor aleatoria aplicada ao Cubo " << sel_idx << std::endl;
                }
                ...
                if(modo_atual == M_OBJ && !cena.empty()) {
                    ...
                    printf("\r[OBJ] Pos: %.2f %.2f %.2f | Rot: %.2f %.2f   ", 
                           cena[sel_idx].posicao.x, cena[sel_idx].posicao.y, cena[sel_idx].posicao.z,
                    ...
                    printf("\r[LUZ] Pos: %.2f %.2f %.2f   ", g_light_pos.x, g_light_pos.y, g_light_pos.z);
                    fflush(stdout);
                }
                    ...
                    printf("\r[CAM] Pos: %.2f %.2f %.2f   ", g_cam_pos.x, g_cam_pos.y, g_cam_pos.z);
                    fflush(stdout);
                }
                else if(modo_atual == M_MAT && !cena.empty()) {
                    // Ka
                    if(e.key.keysym.sym==SDLK_1 || e.key.keysym.sym==SDLK_KP_1) cena[sel_idx].ka = std::min(1.0f, cena[sel_idx].ka + mat_s);
                    if(e.key.keysym.sym==SDLK_2 || e.key.keysym.sym==SDLK_KP_2) cena[sel_idx].ka = std::max(0.0f, cena[sel_idx].ka - mat_s);
                    // Kd
                    if(e.key.keysym.sym==SDLK_3 || e.key.keysym.sym==SDLK_KP_3) cena[sel_idx].kd = std::min(1.0f, cena[sel_idx].kd + mat_s);
                    if(e.key.keysym.sym==SDLK_4 || e.key.keysym.sym==SDLK_KP_4) cena[sel_idx].kd = std::max(0.0f, cena[sel_idx].kd - mat_s);
                    // Ks
                    if(e.key.keysym.sym==SDLK_5 || e.key.keysym.sym==SDLK_KP_5) cena[sel_idx].ks = std::min(1.0f, cena[sel_idx].ks + mat_s);
                    if(e.key.keysym.sym==SDLK_6 || e.key.keysym.sym==SDLK_KP_6) cena[sel_idx].ks = std::max(0.0f, cena[sel_idx].ks - mat_s);
                    // Brilho
                    if(e.key.keysym.sym==SDLK_7 || e.key.keysym.sym==SDLK_KP_7) cena[sel_idx].shininess += 5.0f;
                    if(e.key.keysym.sym==SDLK_8 || e.key.keysym.sym==SDLK_KP_8) cena[sel_idx].shininess = std::max(1.0f, cena[sel_idx].shininess - 5.0f);
                    
                    printf("\r[MAT] Ka: %.2f | Kd: %.2f | Ks: %.2f | Brilho: %.0f   ", 
                           cena[sel_idx].ka, cena[sel_idx].kd, cena[sel_idx].ks, cena[sel_idx].shininess);
                    fflush(stdout);
                }
            }
        }
